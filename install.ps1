...
<#
.SYNOPSIS
    Windows installer for ForgeDash via Docker Desktop.
#>
param(
    [switch]$SkipStart
)
...
Write-Host "=== Installing ForgeDash ===" -ForegroundColor Cyan
...
if (-not $SkipStart) {
    Write-Host "Building and starting ForgeDash on port 9090..." -ForegroundColor Green
    Invoke-Expression "$composeCmd up --build -d"
    Write-Host "ForgeDash installed. Access: http://localhost:9090" -ForegroundColor Green
} else {
